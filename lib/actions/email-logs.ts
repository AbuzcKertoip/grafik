...
export async function triggerManualAlerts() {
    ...
    try {
        ...
        if (medicalExams.length === 0 && cars.length === 0) {
            return { success: true, message: "System sprawdzony. Brak zdarzeń wymagających wysłania alertu na chwilę obecną." }
        }

        // Build HTML Report
        let htmlContent = `
            <h2>Ręczny Raport Alertów - HR4YOU</h2>
            <p>Raport został wygenerowany na żądanie administratora. Poniżej znajduje się lista zdarzeń wymagających uwagi w ciągu najbliższych ${alertDays} dni.</p>
        `
        ...
        htmlContent += `<br/><p><small>Wiadomość wygenerowana z panelu administratora w systemie HR4YOU.</small></p>`

        // Send Email
        const emails = settings.alertEmails.split(',').map((e: string) => e.trim()).filter((e: string) => e)
        for (const email of emails) {
            await sendEmail(email, "HR4YOU - Raport Alertów na żądanie", htmlContent)
        }
